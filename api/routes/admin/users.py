...
def get_users(
    ...
    try:
        ...
        total = base.count()
        users = base.order_by(Patient.patient_id.desc()).offset(offset).limit(limit).all()
...
def create_user(data: AdminPatientCreate, db: Session = Depends(get_db)):
    try:
        ...
        db.commit()
        db.refresh(user)
        return {"message": "User created successfully", "user_id": user.patient_id}
...
@router.put("/{user_id}")
def update_user(user_id: int, data: AdminPatientUpdate, db: Session = Depends(get_db)):
    try:
